    ...
    CommandLineRunner initDataBase(ParkSpotRepository repository) {
        return args -> {
            ...
            if(repository.count()==0){
                ...
                repository.save(new ParkSpot("34TEST01",StreetLocation.TEVFIK_BEY, VehicleType.SMALL, "Merkez", "Bilecik","LEFT"));

                // Cumhuriyet caddesine büyük bir ticari araç
                repository.save(new ParkSpot("06BASKAN99", StreetLocation.CUMHURIYET, VehicleType.LARGE, "Merkez", "Bilecik","RIGHT"));

                // Ali Rıza Özkay caddesine senin adınla bir araç :)
                repository.save(new ParkSpot("11KADIR01", StreetLocation.ALI_RIZA_OZKAY, VehicleType.SMALL, "Merkez", "Bilecik","LEFT"));
