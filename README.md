...
* **Admin Web (The HQ):** A Business Intelligence (BI) dashboard for real-time monitoring and financial reporting, designed with corporate UI standards.
* **Citizen Portal (The Quick Exit):** A web-based, mobile-optimized payment gateway triggered by dynamically generated QR codes for instant self-checkout.
...
* **QR-Based Citizen Checkout:** Field officers can generate a dynamic QR code for any parked vehicle. Citizens scan it to securely view their duration and process payment via a mobile-optimized web portal, triggering an automatic system check-out.
...
| **Mobile** | 💙 **Flutter (Dart)** | Cross-platform mobile app built with SOLID principles & `qr_flutter`. |
| **Web / BI** | 📊 **HTML/JS, Chart.js, SheetJS** | Admin dashboard and Citizen Payment Portal. |
...
Since the backend is deployed in the cloud, you can test the mobile app and web dashboards directly without running a local server.
...
2. **Web Dashboards:** Open `web/admin-panel.html` or `web/customer-payment.html` in your browser.
...
5. Update the `globalBaseUrl` in the Flutter and HTML files to your local machine's IP address.
...
- [x] Phase 11: Self-Service QR Code Payment Portal for Citizens
- [ ] Phase 12: Field Officer Authentication & Role-Based Access ⚡ **(CURRENTLY HERE)**
